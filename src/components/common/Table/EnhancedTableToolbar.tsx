...
import { Icon, Select, SelectOption } from '@/components/base'
...
const EnhancedTableToolbar: FC<EnhancedTableToolbarProps> = ({
  ...
  console.log('initSearchTerm', initialSearchTermType)
        ...
        <Stack direction="row" alignItems="center">
          {onMultipleDelete && (
            <Tooltip title="Delete">
              <IconButton sx={{ mr: 2 }} onClick={() => onMultipleDelete()}>
                <Icon
                  type="fill"
              ...
              ({ src, tooltip, handleOnClick }, idx) => (
                <Tooltip title={tooltip} key={idx}>
                  <IconButton sx={{ mr: 2 }} onClick={() => handleOnClick()}>
                    <Icon type="fill" color={colorPalette.dark} src={src} />
                  </IconButton>
                </Tooltip>
              )
            )}
